...
export class OpenRouterAdapter implements IAIService {
  ...
  async analyzeMessage(text: string): Promise<{ type: 'NECESIDAD' | 'BRAIN'; summary: string; model: string }> {
    ...
    let sanitizedType = typeof content.type === "string" ? content.type.toUpperCase().trim() : "BRAIN";
    if (sanitizedType !== "NECESIDAD" && sanitizedType !== "BRAIN") {
      sanitizedType = "BRAIN";
    }

    const result = {
      type: sanitizedType as 'NECESIDAD' | 'BRAIN',
