...
export class TelegramAdapter {
  ...
  private setupHandlers() {
    ...
    // Escuchar actualizaciones de la encuesta (votos en tiempo real) y su cierre
    this.bot.on("poll", async (ctx) => {
      const poll = ctx.poll;
      
      try {
        const validation = await this.fulfillFavor.getValidation(poll.id);
        if (!validation) return; // Ya se resolvió o no existe en DB

        const yesVotes = poll.options[0].voter_count;
        const noVotes = poll.options[1].voter_count;

        if (!poll.is_closed) {
          // Encuesta abierta: Chequear mayoría en tiempo real
          const memberCount = await ctx.api.getChatMemberCount(validation.chatId);
          const threshold = Math.floor(memberCount / 2) + 1;

          console.log(`🗳️ Actualización poll ${poll.id}: Yes=${yesVotes}, No=${noVotes}, Threshold=${threshold}`);

          if (yesVotes >= threshold) {
            // ¡Mayoría alcanzada! Completar favor inmediatamente pero dejar la encuesta fluir.
            const favor = await this.fulfillFavor.getFavorById(validation.favorId);
            if (favor && favor.status === "PENDING") {
              await this.fulfillFavor.resolveValidation(poll.id, true);
              await this.bot.api.sendMessage(validation.chatId, `✅ **Favor Validado por Mayoría** (${yesVotes}/${memberCount})\nLa comunidad ha confirmado la tarea rápidamente. ¡Puntos asignados! (La encuesta seguirá abierta para que el resto pueda votar).`);
            }
          }
        } else {
          // Encuesta cerrada (por tiempo o terminada manualmente)
        ...
        console.error("Error al procesar actualización de la encuesta:", error);
      }
    });
